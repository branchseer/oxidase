use std::{
    cmp::max, fs::{self, read_to_string}, path::{Path, PathBuf}, sync::atomic::{AtomicU64, Ordering}, time::SystemTime
};
...
impl BaselineCache {
    pub fn new(cache_root: impl Into<PathBuf>) -> Self {
        ...
        if let Ok(reader) = csv::ReaderBuilder::new().has_headers(false).from_path(list_path(&cache_root)) {
    ...
    pub fn get_or_insert_with(
        ...
        if let Some(entry) = self.entries.get(path) {
            ...
            if entry.mtime == mtime {
                ...
                let js =
                    read_to_string(self.js_path(entry.id)).unwrap();
                let ts =
                    read_to_string(self.ts_path(entry.id)).unwrap();
        ...
        self.entries.insert(path.to_owned(), if let Some(output) = &output {
            ...
            CacheEntry { id: 0, mtime, source_kind: None }
        });
        output
    }
    pub fn save(self) {
        let mut csv_writer = csv::WriterBuilder::new().has_headers(false).from_path(list_path(&self.cache_root)).unwrap();
